       ...
       DO n=1,nMax
          if (mask(n) /= 0) then
             ...
             if (add_gusts) then 
                ...
                vmag   = max(seq_flux_atmocn_minwind, sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2 + (0.0_R8*ugust(min(rainc(n),6.94444e-4_r8))**2)) )
                ugust_out(n) = ugust(min(rainc(n),6.94444e-4_r8))
             else 
                vmag   = max(seq_flux_atmocn_minwind, sqrt( (ubot(n)-us(n))**2 + (vbot(n)-vs(n))**2) )
                ugust_out(n) = 0.0_r8
             end if
             !write(iulog,*)'  MDF:   Value of ugust = ',ugust(min(rainc(n),6.94444e-4_r8))
             !write(iulog,*)'  MDF:   rainc = ',rainc(n)
             !write(iulog,*)'  MDF:   vmag = ',vmag
             ! --- MDF 
