...
!+++arh
    real( kind = core_rknd ), dimension(ngrdcol), intent(in) :: pblh
...
!+++arh
    integer, dimension(ngrdcol) :: kref

!+++arh                             
    real( kind = core_rknd ), dimension(ngrdcol) :: zref

!+++arh
    real( kind = core_rknd ) :: scale_exp = 3.0_core_rknd
...
!+++arh
        !Km_zm_p_nu10_scaled(i,k) = Km_zm(i,k) + nu_vert_res_dep%nu10(i)
      end do
    end do
    !$acc end parallel loop
...
!+++arh - generate Kmh_zm_scaled profiles
      kref(:) = 1
      zref(:) = 1000.0_core_rknd
      !zref(:) = pblh(:)
      do k = 1, nz
        do i = 1, ngrdcol
          if (gr%zm(i,k) <= zref(i)) then
            kref(i) = k
          end if
        end do
      end do

      Kmh_zm_scaled(:,:) = Kmh_zm(:,:)
      do i = 1, ngrdcol
        do k = 1, kref(i)
          Kmh_zm_scaled(i,k) = Kmh_zm_scaled(i,k) * (one - ( (zref(i) - gr%zm(i,k))/zref(i) )**scale_exp)
        end do
      end do
!---arh
...
!+++arh
      Kmh_zt_scaled(:,:) = zm2zt( nz, ngrdcol, gr, Kmh_zm_scaled(:,:), zero )
...
!+++arh
      ! Calculate diffusion terms
      call diffusion_zt_lhs( nz, ngrdcol, gr, Kmh_zm_scaled, Kmh_zt_scaled, nu_zero,  & ! intent(in)
                             invrs_rho_ds_zt, rho_ds_zm,                & ! intent(in)
                             lhs_diff_scaled )
