...
!---ARH
   ...
   real(r8) :: lat_column_degree ! to pass lats into CLUBB
...
!+++ARH
      jctop_tmp = jctop(i)
      jctop_tmp = pver-jctop_tmp+2

      !if (masterproc) write(200+iam,*) "jctop flipped: ", jctop_tmp
!---ARH
      ...
      do t=1,nadv    ! do needed number of "sub" timesteps for each CAM step
         ...
         lat_column_degree = 180._r8 * state%lat(i) / 3.14159265358979_r8
