...
!-------------------------------------------------------------------------------
    ...
    call pbuf_add_field('DUDZ',    'global', dtype_r8, (/pcols,pverp,dyn_time_lvls/), dudz_cam_idx)
    call pbuf_add_field('DVDZ',    'global', dtype_r8, (/pcols,pverp,dyn_time_lvls/), dvdz_cam_idx)
...
!$OMP END PARALLEL
    ...
    if (is_first_step()) then
       ...
       call pbuf_set_field(pbuf2d, dudz_cam_idx,    0.0_r8)
       call pbuf_set_field(pbuf2d, dvdz_cam_idx,    0.0_r8)
  ...
  !-------------------------------------------------------------------------------
    ...
    real(r8), pointer, dimension(:,:) :: dudz_cam     ! east-west momentum flux                     [m^2/s^2]
    real(r8), pointer, dimension(:,:) :: dvdz_cam     ! north-south momentum flux                   [m^2/s^2]
   ...
   logical :: min_lev_not_found
    ...
    call pbuf_get_field(pbuf, dudz_cam_idx,   dudz_cam,    start=(/1,1,itim_old/), kount=(/pcols,pverp,1/))
    call pbuf_get_field(pbuf, dvdz_cam_idx,    dvdz_cam,    start=(/1,1,itim_old/), kount=(/pcols,pverp,1/))
    ...
    if ( clubb_l_predict_upwp_vpwp ) then
      ...
      upwp_height_ug = 0
      vpwp_height_ug = 0
      dudz = ddzt( nlev+1, ncol, gr, um_in )
      dvdz = ddzt( nlev+1, ncol, gr, vm_in )
      do k=1, nlev+1
        do i=1, ncol
