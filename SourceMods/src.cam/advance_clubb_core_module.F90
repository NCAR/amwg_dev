...
!++jtb
    ! Host model column latititude
    real( kind = core_rknd ), intent(in) :: lat_column_degree
!--jtb
...
!++jtb
      ...
      Kh_zm_thermo = 0._core_rknd * p_in_pa_zm

      if ( abs(lat_column_degree) < 20.) then
         where( (p_in_pa_zm < 50000._core_rknd).AND. &
                (p_in_pa_zm > 5000._core_rknd) )
          Kh_zm_thermo = 10._core_rknd
         end where
      end if
...
!                           Lscale, wp3_on_wp2, wp3_on_wp2_zt, Kh_zt, Kh_zm, & ! intent(in)
                            Lscale, wp3_on_wp2, wp3_on_wp2_zt, Kh_zt, Kh_zm_thermo, & ! intent(in)
!--jtb
