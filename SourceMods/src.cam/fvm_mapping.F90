...
!    iwidth=1
  ...
  subroutine dyn2phys_all_vars(nets,nete,elem,fvm,&
    ...
    do ie=nets,nete
      ...
      phis_phys(:,ie) = RESHAPE(fvm(ie)%phis_physgrid,SHAPE(phis_phys(:,ie)))!old
      phis_phys(:,ie) = RESHAPE(dyn2phys(elem(ie)%state%phis(:,:),elem(ie)%metdet(:,:),inv_area),SHAPE(phis_phys(:,ie)))!new
      ps_phys(:,ie) = ptop      
      if (nc.ne.fv_nphys) then
        tmp = 1.0_r8
        do k=1,nlev
