...
!   variables used to calculate CFL
    ...
    real (kind=r8) :: ps_ref(np,np,nets:nete), dp0
     ...
     do ie=nets,nete
       do k=1,nlev
         ...
         if (hvcoord%hybm(k)>0) then
           ...
           !
           ! pel@ucar.edu: resolved noise issue over Antartica - bug not fixed in CESM2.2 release branch
           !
           elem(ie)%derived%dp_ref(:,:,k)   = elem(ie)%derived%dp_ref(:,:,k)-dp0
         else
           elem(ie)%derived%T_ref(:,:,k)    = 0.0_r8
           elem(ie)%derived%dp_ref(:,:,k)   = 0.0_r8
         end if
       end do
     end do
